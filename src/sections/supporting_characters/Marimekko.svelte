<script lang="ts">
  ...
  const supportingCharsScreenTime = charsScreenTime;
  ...
  let totalBarsScreenTime = $derived(
    supportingCharsScreenTime.reduce((acc, char) => acc + char.screenTime, 0)
  );
  ...
  const laughsScale = $derived(
    scaleLinear()
      .domain([0, Math.max(...supportingCharsScreenTime.map((char) => char.onScreenWithoutLaughs))])
      .range([0, chartHeight / 2 - 8])
  );

  const charsData = $derived.by(() => {
    const array = supportingCharsScreenTime;
    array.forEach((char, i) => {
      char['screenTimeWidth'] = screenTimeScale(char.screenTime);
      char['paddingLeft'] = i === 0 ? 0 : supportingCharsScreenTime.slice(0, i).reduce((acc, c) => acc + c.screenTimeWidth + 50, 0);
